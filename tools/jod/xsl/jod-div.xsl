...
<xsl:template match="columnbreak">
    <xsl:element name="div"><xsl:attribute
        name="class">break columnbreak</xsl:attribute>
        <xsl:attribute name="id"><xsl:value-of select="@No" />b</xsl:attribute><xsl:element
        name="a"><xsl:attribute
        name="name"><xsl:value-of
        select="@No"/>b</xsl:attribute></xsl:element><![CDATA[[]]><xsl:element
        name="a"><xsl:attribute
        ...
        name="target">_pdfwin</xsl:attribute>p.<xsl:text
        disable-output-escaping="yes">&amp;nbsp;</xsl:text><xsl:value-of
        select="@No"/>b<![CDATA[]]]><xsl:element
        name="img"><xsl:attribute
        name="class">im</xsl:attribute><xsl:attribute
        name="src">pdf.png</xsl:attribute></xsl:element></xsl:element><xsl:element
        name="a"><xsl:attribute
        ...
        name="target">_pdfwin2</xsl:attribute><xsl:element
        name="img"><xsl:attribute
        name="class">im</xsl:attribute><xsl:attribute
        name="src">hbll.png</xsl:attribute></xsl:element></xsl:element>
    </xsl:element>
</xsl:template>
...
<xsl:template match="Page">
    <xsl:element name="div"><xsl:attribute
        ...
        name="id"><xsl:value-of select="@No" />a</xsl:attribute><xsl:element
        name="a"><xsl:attribute
        name="name"><xsl:value-of
        select="@No"/>a</xsl:attribute></xsl:element><![CDATA[[]]><xsl:element
        name="a"><xsl:attribute
        name="href">/jod/pdf/JoD<xsl:value-of
        select="@Vol"/>/JoD<xsl:value-of
        select="@Vol"/>_0<xsl:value-of
